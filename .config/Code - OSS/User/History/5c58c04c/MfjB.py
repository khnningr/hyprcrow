...
class GestorTareas():
    ...
    def guardar_tareas_txt(
        ...
        for tarea in tareas: 
            tareas_guardadas.write(
                ...
                + tarea.get_responsable() + " | "
                + tarea.get_prioridad() + " | "
                + tarea.get_estado() + " | "
                + tarea.get_nombre_usuario() + " | "
                + tarea.get_apellido()
                + "\n"
            )
        tareas_guardadas.close()
        print("\n» Guardando copia de respaldo de las tareas actuales. «")
    def formatear_tareas_txt(
        ...
        tareas_guardadas.close()
        print("\n» Archivo actual de respaldo de las tareas formateado. «")
    def cargar_tareas_txt(self):
        tareas = []
        try:
            tareas_guardadas = open("tareas.txt", "r")
            for tarea in tareas_guardadas:
                # Strip y split me permiten filtrar para dar el formato que necesito.abs

                datos = tarea.strip().split(" | ")
                print(datos)
                if len(datos) == 8:
                    tarea = Tarea(
                        nombre_tarea=datos[0],
                        id_tarea=datos[1],
                        descripcion=datos[2],
                        responsable=datos[3],
                        prioridad=datos[4],
                        estado=datos[5],
                        nombre_usuario=datos[6],
                        apellido=datos[7]
                    )
                    tareas.append(tarea)
            tareas_guardadas.close()
        except FileNotFoundError:
            print("El archivo 'tareas.txt' no se encontró.")
        print("\n» Carga completa de la copia de respaldo de las tareas actuales «")
        return tareas
    ...
    def menu_principal(
        ...
        menu_principal = (
            ...
            "Generar reporte",
            "Cargar tareas guardadas desde txt",
            "Formatear tareas guardadas en txt",
            "Guardar tareas en txt"
        )
        while True:
            ...
            elif "Cargar tareas guardadas desde txt" in elegir:
                lista_de_tareas = self.cargar_tareas_txt()
            elif "Formatear tareas guardadas en txt" in elegir:
                self.formatear_tareas_txt()
            elif "Guardar tareas en txt" in elegir:
                self.guardar_tareas_txt(
                    tareas = lista_de_tareas
                )
