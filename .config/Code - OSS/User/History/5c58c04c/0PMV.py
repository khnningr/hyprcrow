...
class GestorTareas():
    ...
    def cargar_tareas_txt(self):
        tareas = []
        try:
            with open("tareas.txt", "r") as tareas_guardadas:
                for linea in tareas_guardadas:
                    # Eliminar el salto de línea y dividir la línea por el separador "|"
                    datos = linea.strip().split(" | ")
                    if len(datos) == 8:  # Asegúrate de que hay 8 elementos
                        tarea = Tarea(
...
if __name__ == "__main__":
    ...
    tareas = gestor.cargar_tareas_txt()

    for i in tareas:
        print(i)
        print(i.get_nombre_tarea())
        print(i.get_nombre_usuario())
        print(i.get_apellido())
