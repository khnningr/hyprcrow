...
def mostrar_conjuto_datos_bidimencional(titulo, matriz, formulario="",contador=1,salir = "", elegir_opcion=""):
    establecer_titulo(titulo)
...
def prioridades_disponibles(prioridad = "- Sin prioridad -"):
    ...
    if "Alta" in prioridad or "Media" in prioridad or "Baja" in prioridad:
        ...
        if cambiar != "S" and cambiar != "N" or cambiar == "N":
            print(prioridad)
            return prioridad      
    prioridad = mostrar_conjuto_datos_unidimensional("Prioridades", prioridades,pregunta="Seleccione una prioridad:")
    return prioridad
...
def registrar_tarea(nombre_tarea = "- Sin nombre -",
    ...
    if mostrar_registro == "S":
        mostrar_conjuto_datos_unidimensional("Tarea registrada", registrar_tarea, formulario_tarea)
...
def consultar_tareas():
    formulario_tarea = ["Nombre: ", "Estado: ", "Prioridad: ",
                   "Responsable: ", "Descripción:" ]
    if tareas:
        tarea = mostrar_conjuto_datos_bidimencional(titulo="Tareas en general", matriz=tareas, elegir_opcion="Seleccionar una tarea:")
        mostrar_conjuto_datos_unidimensional(titulo="Prueba",mostrar_contenedor=tarea , formulario=formulario_tarea)
